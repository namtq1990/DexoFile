...
void NcLibrary::AdaptFilter(const Spectrum *in, Spectrum *out, const FHM &fhm, const Coeffcients* coeffs,
                            const nucare::uint repeat)
{
    bool byChannel = coeffs == nullptr;
    const nucare::uint n = in->getSize();
    ...
    memcpy(data, dataIn, n * sizeof(double));
    memset(dataOut, 0, n * sizeof(double));

    for (nucare::uint nosmoo = 1; nosmoo <= repeat; nosmoo++)
    {
        if (nosmoo > 1)
        {
            memcpy(data, dataOut, n * sizeof(double));
        }
        ...
        for (nucare::uint j = 3; j < n - temp_wind; j++)
        {
            ...
            nucare::uint start = max(0, (int) j - wnd_half);
            nucare::uint end = min(j + wnd_half + 1, byChannel ? n : CHSIZE);
...
std::array<double, 2> NcLibrary::FindPeak(Spectrum *spc, const std::array<int, 2>& peakInfo)
{
    array<double, 2> ret;
    ...
    try {
        ...
        memcpy(y, data, spc->getSize() * sizeof(double));
        ...
        memset(ret.data(), 0, ret.size() * sizeof(double));
        return ret;
    }
